...
import api.endpoints.EndPoints_propertiesFile;
...
public class UserTest2 {
	...
	Response response=	EndPoints_propertiesFile.createUser(userPayload);
...
@Test(priority=2)
	...
	System.out.println("Trying to get user with username: " + userPayload.getUsername());
		Response response=EndPoints_propertiesFile.readUser(userPayload.getUsername());
		response.then().log().all();
		AssertJUnit.assertEquals(response.getStatusCode(),200);
...
void testUpdateUserByName()
{
	...
	Response response=EndPoints_propertiesFile.updateUser(userPayload, this.userPayload.getUsername());
	response.then().log().all();
	...
	Response response_after_update = EndPoints_propertiesFile.readUser(this.userPayload.getUsername());
...
Response response=	EndPoints_propertiesFile.deleteUser(this.userPayload.getUsername());  //this keyword is used to refer to the
//current user name
AssertJUnit.assertEquals(response.getStatusCode(),200); 
